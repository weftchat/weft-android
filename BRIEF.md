...
- **Tres códigos (Fase 2, confirmado por el dueño):** PIN real, PIN de coacción y código de pánico, todos de 6 dígitos. Ningún código se guarda: un archivo de tres casillas iguales guarda, por casilla, una etiqueta y la llave del perfil sellada con una clave del Android Keystore distinta por casilla; las casillas sin usar son datos al azar. Cada intento hace el mismo trabajo, acierte o no.
- **Perfil señuelo (confirmado por el dueño):** es un segundo perfil real que funciona por el relay del usuario, en su propia base SQLCipher. Las carpetas de los dos perfiles tienen nombres neutros y, mientras no hay señuelo, la suya se rellena con datos al azar del mismo tamaño. El señuelo se crea desde Security al activar el PIN de coacción (necesita relay propio) y empieza vacío: el dueño lo llena desde dentro. En el señuelo, Security muestra la coacción y el pánico apagados.
- **Código de pánico (confirmado por el dueño):** abre el señuelo con los mismos pasos y tiempos que un desbloqueo normal y, ya abierto, destruye en segundo plano la clave del Keystore del perfil real y después sus archivos. Luego el PIN real se comporta como uno equivocado y cuenta para los 10 intentos.
- **Borrado:** siempre destruye primero las claves del Keystore (sin ellas la base de datos es ruido) y después los archivos. Tras 10 PIN fallidos, opción apagada por defecto. El texto del diseño "pulsa el botón lateral 5 veces" no es posible para una app; esa fila es el código de pánico.
- **Capturas (confirmado por el dueño):** FLAG_SECURE en toda la app, siempre; en Security aparece como "always on", no como interruptor.
- **Bloqueo automático (confirmado por el dueño):** al salir de la app, tras Inmediato / 30 s / 1 min / 5 min / 15 min; por defecto 1 min.
- **Guardia USB:** siempre activa. Con la app en segundo plano o bloqueada, una conexión USB de datos (no solo carga) bloquea, borra la clave de la memoria y termina el proceso. "Wipe on USB data" es la opción avanzada, apagada por defecto y con aviso. Solo actúa mientras el proceso de Weft está vivo; por eso se recomienda el ajuste de puerto USB-C de GrapheneOS.
